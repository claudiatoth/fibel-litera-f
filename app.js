/* ===========================================================
   FIBEL — „Litera F cu Herzchen" · Claudia Toth © 2026
   Abecedar german interactiv pentru copii 4–8 ani.
   Temă: Fahrrad 🚲 · „F wie Fahrrad" · „Herzchen fährt Fahrrad".

   17 secțiuni: Start · Litera F · Suche F · Cuvinte · Citește (cuvinte+propoziții) ·
   Legare silabe (unește) · 🆕 Despărțire în silabe (clap) · Joc F · Wo hörst du F ·
...
const F = {
  ...
  katze: {de:'die Katze', icon:'katze', audio:'f-katze', gen:'f', art:'die', ro:'pisica',    m:false},
  apfel: {de:'der Apfel', icon:'apfel', audio:'f-apfel', gen:'m', art:'der', ro:'mărul',     m:false},
  obst:  {de:'das Obst',  icon:'obst',  audio:'f-obst',  gen:'n', art:'das', ro:'fructele',  m:false},
  tiger: {de:'der Tiger', icon:'tiger', audio:'f-tiger', gen:'m', art:'der', ro:'tigrul',    m:false},
};
const WORDS_F  = ['fahrrad','fisch','frosch','feder','fenster','fuss'];
const GAME_POOL= ['fahrrad','maus','fisch','ball','frosch','katze'];
const ART_QUIZ = ['fahrrad','fisch','feder','fenster'];
const GRID = [
  ['F','m','f','N','b'],['l','f','M','F','n'],['F','o','f','d','F'],['t','F','a','f','B'],['d','f','M','F','w'],
];
/* despărțire în silabe (clap) — 1 → 2 → 3 silabe */
const SPLIT = [
  {icon:'fisch',   audio:'f-fisch',   syl:['Fisch'],        sylA:['f-syl-fisch']},
  {icon:'fahrrad', audio:'f-fahrrad', syl:['Fahr','rad'],   sylA:['f-syl-fahr','f-syl-rad']},
  {icon:'elefant', audio:'f-elefant', syl:['E','le','fant'],sylA:['f-syl-e','f-syl-le','f-syl-fant']},
];
const STORY = [
  {de:'Das ist <b class="m-bold">Herzchen</b>.',                              ro:'Acesta e Herzchen (inimioara).', icon:'herzchen', audio:'st-1'},
  {de:'<b class="m-bold">Herzchen</b> sieht ein <b class="m-bold">Fahrrad</b>.',ro:'Herzchen vede o bicicletă.',    icon:'fahrrad',  audio:'st-2'},
  {de:'<b class="m-bold">Herzchen</b> fährt <b class="m-bold">Fahrrad</b>.',   ro:'Herzchen merge cu bicicleta.',  icon:'fahrrad',  audio:'st-3'},
  {de:'<b class="m-bold">Herzchen</b> ist glücklich.',                        ro:'Herzchen e fericit.',           icon:'herzchen', audio:'st-4'},
];
const QUIZ = [
  {type:'audio',   ans:'fahrrad', opts:['fahrrad','maus','ball']},
  {type:'findm',   ans:'fisch',   opts:['fisch','katze','mond']},
  {type:'article', word:'fenster'},
  {type:'audio',   ans:'frosch',  opts:['frosch','apfel','tiger']},
  {type:'findm',   ans:'feder',   opts:['feder','ball','obst']},
];
const LETTER = 'F';
const POS = [
  {icon:'fahrrad', audio:'f-fahrrad', pos:'a'},
  {icon:'fisch',   audio:'f-fisch',   pos:'a'},
  {icon:'frosch',  audio:'f-frosch',  pos:'a'},
  {icon:'feder',   audio:'f-feder',   pos:'a'},
];
...
SCREENS[0] = function(){
  ...
  show(
    ...
    '<h1 class="title">Litera F cu Herzchen</h1>' +
  ...
  document.getElementById('startBtn').onclick = ()=>{ play('f-wie-fahrrad'); go(1); };
...
SCREENS[1] = function(){
  show(
    ...
    '<div class="letter-hero">F<span class="lo">f</span></div>' +
    '<div class="btn-row">' +
      '<button class="big-btn" id="bName">🔊 F</button>' +
      '<button class="big-btn amber" id="bMaus">🔊 F wie Fahrrad</button>' +
    '</div>' +
    '<p class="parent-note">Atinge: „F" — litera se citește „ef" · „F wie Fahrrad" = F ca în Fahrrad (bicicletă).</p>' +
    nextBtn()
  );
  document.getElementById('bName').onclick = ()=>play('f-name');
  document.getElementById('bMaus').onclick = ()=>play('f-wie-fahrrad');
  setTimeout(()=>play('f-wie-fahrrad'), 350);
  wireNext(2);
};
...
SCREENS[2] = function(){
  ...
  GRID.forEach(row=> row.forEach(ch=>{
    const isM = (ch==='F'||ch==='f'); if(isM) total++;
    cells += '<div class="lcell" data-m="'+(isM?1:0)+'">'+ch+'</div>';
  }));
  show(
    '<p class="kicker">Suche F und f 🔍</p>' +
    '<h1 class="title" style="font-size:1.4rem">Atinge toți F și f</h1>' +
...
SCREENS[4] = function(){
  const READ = [
    {t:'Fenster', a:'f-art-fenster'}, {t:'Finn', a:'f-finn'}, {t:'Emil', a:'f-emil'},
    {t:'Nina', a:'f-nina'}, {t:'Oma', a:'f-oma'}, {t:'Tom', a:'f-tom'}
  ];
  const SENT = [
    {t:'Finn rennt.', a:'st-finnrennt'}, {t:'Emil rennt.', a:'st-emilrennt'}
  ];
  ...
  show(
    ...
    '<h1 class="title" style="font-size:1.1rem">Cu literele învățate + F</h1>' +
...
SCREENS[5] = function(){
  ...
  const VALID = {
    'No|ra': {w:'Nora', a:'f-nora'},
    'Ni|na': {w:'Nina', a:'f-nina'},
    'Mo|na': {w:'Mona', a:'f-mona'},
    'O|ma':  {w:'Oma',  a:'f-oma'}
  };
...
/* 7 — JOC: beginnt mit F? */
SCREENS[7] = function(){
  let tiles = spreadShuffle(GAME_POOL, k=>F[k].m, 2).map(k=>
    '<div class="tile" data-k="'+k+'" style="position:relative"><img src="'+ICON+F[k].icon+'.svg" alt="'+F[k].de+'"></div>'
  ).join('');
  show(
    hero('small') +
    '<p class="game-q">Atinge ce începe cu <span class="m-hi">F</span>!</p>' +
    '<div class="game-grid">'+tiles+'</div>' +
    '<p class="parent-note">Welches Bild beginnt mit F?</p>'
  );
  let found = 0; const need = GAME_POOL.filter(k=>F[k].m).length;
  stage.querySelectorAll('.tile').forEach(el=>{
    el.onclick = function(){
      const w = F[el.dataset.k];
  ...
  function card(){
    ...
    show(
      ...
      '<div class="flashnav">' +
        ...
        '<span class="counter">'+(i+1)+' / '+WORDS_F.length+'</span>' +
    ...
    nx.onclick = ()=>{ if(i<WORDS_F.length-1){ i++; card(); } };
    wireNext(12);
  }
  card();
};

/* 12 — TRASEAZA F / f — cu SĂGEȚI */
SCREENS[12] = function(){
  let glyph = 'F';
  const STROKE = {'F':'stroke-big','f':'stroke-small'};
  function render(){
    show(
      '<p class="kicker">Schreiben · ① urmează săgețile verzi ✏️</p>' +
      '<div class="btn-row" style="margin-bottom:2px">' +
        '<button class="big-btn'+(glyph==='F'?'':' amber')+'" id="gM">F mare</button>' +
        '<button class="big-btn'+(glyph==='f'?'':' amber')+'" id="gm">f mic</button></div>' +
    ...
    document.getElementById('gM').onclick=()=>{glyph='F';render();};
    document.getElementById('gm').onclick=()=>{glyph='f';render();};
...
/* 14 — SCRIE PROPOZIȚIA */
SCREENS[14] = function(){
  const GLYPH = {'N':'s-cap-n','S':'s-cap-s','R':'s-cap-r','L':'s-cap-l','M':'s-cap-m','T':'s-cap-t','O':'s-cap-o','E':'s-cap-e','F':'s-cap-f',
                 'n':'s-n','s':'s-s','r':'s-r','t':'s-t','m':'s-m','a':'s-a','i':'s-i','l':'s-l','o':'s-o','e':'s-e'};
  const SENT = [
    {t:'Finn rennt.', a:'st-finnrennt'},
    {t:'Emil rennt.', a:'st-emilrennt'}
  ];
  ...
  function q(){
    ...
    if (it.type==='audio'){
      body = '<p class="game-q">Ascultă 🔊 și atinge imaginea</p>' +
        ...
        '<div class="opt-grid">'+ shuffle(it.opts).map(k=>'<div class="opt" data-k="'+k+'"><img src="'+ICON+F[k].icon+'.svg" alt=""></div>').join('') +'</div>';
    } else if (it.type==='findm'){
      body = '<p class="game-q">Care începe cu <span class="m-hi">F</span>?</p>' +
        '<div class="opt-grid">'+ shuffle(it.opts).map(k=>'<div class="opt" data-k="'+k+'"><img src="'+ICON+F[k].icon+'.svg" alt=""></div>').join('') +'</div>';
    } else {
      const w = F[it.word];
    ...
    if (it.type==='audio'){
      ...
      stage.querySelectorAll('.opt').forEach(el=> el.onclick = function(){
        if (el.dataset.k === it.ans){ el.classList.add('good'); play(F[el.dataset.k].audio); win(); } else wrong(el);
      });
    } else if (it.type==='findm'){
      stage.querySelectorAll('.opt').forEach(el=> el.onclick = function(){
        if (F[el.dataset.k].m){ el.classList.add('good'); play(F[el.dataset.k].audio); win(); } else wrong(el);
      });
    } else {
      const w = F[it.word]; play(w.audio);
...
SCREENS[16] = function(){
  show(
    ...
    '<div class="story" style="max-width:380px"><div class="story-row read" id="rima">' +
      '<img src="'+ICON+'fahrrad.svg" alt="">' +
      '<div><div class="s-de"><b class="m-bold">F</b> wie <b class="m-bold">Fahrrad</b>.<br><b class="m-bold">Herzchen</b> fährt <b class="m-bold">Fahrrad</b>.</div>' +
      '<div class="s-ro">F ca în Fahrrad. Herzchen merge cu bicicleta.</div></div>' +
...
function printDiploma(){
  ...
  document.getElementById('diploma-print').innerHTML =
    '<div class="dip">' +
      ...
      '<div class="dip-letter">F f</div>' +
      '<div class="dip-text">F wie Fahrrad — mit Herzchen! 🧡</div>' +
...
function printColoring(){
  ...
  document.getElementById('coloring-print').innerHTML =
    '<div class="color-page">' +
      ...
      '<h2 class="cp-title">F — Herzchen fährt Fahrrad</h2>' +
      '<img src="'+ICON+'herzchen-color.svg" alt="Herzchen fährt Fahrrad">' +
      '<div class="cp-sentence">Herzchen fährt Fahrrad.</div>' +
...
(function initParent(){
  ...
  if (wl) wl.innerHTML = WORDS_F.map(k=>'<span>'+F[k].de+'</span>').join('');
